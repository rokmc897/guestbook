...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="rokmc897/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/rokmc897/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'rokmc897.j@gmail.com')
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) rokmc897 빌드가 성공적으로 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        failure { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'danger'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) rokmc897 빌드가 실패하였습니다. Details: (<${BUILD_URL} | here >)")
    }
  }
}
